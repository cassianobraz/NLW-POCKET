    ...
    <div className='py-10 max-w-[480px] mx-auto px-5 flex flex-col gap-6'>
      ...
      <div className='flex flex-col gap-3'>
        ...
        <div className='flex items-center justify-between text-xs text-zinc-400'>
          <span>Você completou <span className='text-zinc-100'>{data.completed}</span> de <span className='text-zinc-100'>{data.total}</span> metas nessa semana.
          </span>
        ...
        {Object.entries(data.goalsPerDay).map(([ date, goals ]) => {
          ...
          return (
            <div key={date} className='flex flex-col gap-4'>
              <h3 className='font-medium'><span className='capitalize'>{weekDay}{' '}</span>
